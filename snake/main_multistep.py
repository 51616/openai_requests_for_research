...
from model import feedforward, convnet, ResNet
...
policy_net = ResNet(config.BOARD_SIZE, in_channel=5).float().to(device, non_blocking=True).eval()
target_net = ResNet(config.BOARD_SIZE, in_channel=5).float().to(device, non_blocking=True).eval()
